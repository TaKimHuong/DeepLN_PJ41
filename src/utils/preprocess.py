...
if __name__ == "__main__":
    ...
    for dataset_version in ["data_01", "data_03"]:
        print(f"\n>>> PROCESSING: {dataset_version}")
        
        # Xác định đường dẫn file input
        dataset_path = f"data/{dataset_version}/raw/public_train.csv"
        if not os.path.exists(dataset_path):
            dataset_path = f"data/{dataset_version}/public_train.csv"
        ...
        # Save processed data
        output_dir = f"data/{dataset_version}/processed"
        os.makedirs(output_dir, exist_ok=True)
        
        train_df.to_csv(os.path.join(output_dir, "train.csv"), index=False)
        val_df.to_csv(os.path.join(output_dir, "val.csv"), index=False)
        print(f"DONE: Processed data for {dataset_version} saved to {output_dir}")
