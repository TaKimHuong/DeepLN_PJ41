...
def compute_metrics(pred):
    ...
    # [LOGITS] - pred.predictions chứa kết quả thô từ mô hình PhoBERT
    # [ARGMAX] - Lấy vị trí (index) có giá trị lớn nhất để quyết định nhãn 0 hoặc 1
    preds = pred.predictions.argmax(-1)
    acc = accuracy_score(labels, preds)
    prec = precision_score(labels, preds, average='weighted', zero_division=0)
    rec = recall_score(labels, preds, average='weighted', zero_division=0)
    f1 = f1_score(labels, preds, average='weighted', zero_division=0)
    # [STEP 5 & 6: PREDICTION & EVALUATION] - Dự đoán và tính toán các chỉ số chất lượng
    return {
        'accuracy': acc,
        'precision': prec,
        'recall': rec,
...
def run_phobert_experiment(dropout, batch_size, learning_rate, train_texts, train_labels, val_texts, val_labels, best_score=-1.0):
    ...
    # [STEP 2: TOKENIZATION] - Phân mảnh và mã hóa văn bản bằng BPE
    ...
    # [STEP 3: MODEL ARCHITECTURE] - Tải mô hình PhoBERT đã được pre-trained
    model.config.id2label = LABEL_MAPPING
    model.config.id2label = LABEL_MAPPING
    model.config.label2id = {"Real": 0, "Fake": 1}
    
    # --- NEW: APPLY LoRA TECHNIQUE ---
    if PEFT_AVAILABLE:
        ...
        print("[WARNING] PEFT library not found. Falling back to Feature Extraction (Frozen Weights).")
        # Legacy: Unfrozen PhoBERT encoder layers for full fine-tuning
    ...
    training_args = build_training_args(
        ...
        fp16=torch.cuda.is_available(), # Enable Mixed Precision for 2x speed
    ...
    # [STEP 4: FINE-TUNING] - Quá trình tinh chỉnh mô hình trên tập tin giả
...
if __name__ == "__main__":
    ...
    train_path = os.path.join(BASE_PATH, "data/data_03/processed/train.csv")
    val_path = os.path.join(BASE_PATH, "data/data_03/processed/val.csv")
    
    if not os.path.exists(train_path):
        train_path = "data/data_03/processed/train.csv"
        val_path = "data/data_03/processed/val.csv"
