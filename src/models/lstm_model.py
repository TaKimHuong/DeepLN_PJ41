...
class FakeNewsDataset(Dataset):
    # [STEP 2: TOKENIZATION] - Biến văn bản thành dãy số ID dựa trên từ điển
...
# [STEP 3: MODEL ARCHITECTURE] - Định nghĩa cấu trúc mạng LSTM
...
def train_model(model, train_loader, val_loader, optimizer, criterion, epochs=10):
    ...
    # [STEP 4: TRAINING LOOP] - Quá trình học và tối ưu trọng số
    for epoch in range(epochs):
        ...
        for texts, labels in train_loader:
            ...
            optimizer.zero_grad()
            # Forward Pass: Đưa dữ liệu qua Model
            # [LOGITS] - Kết quả thô từ mô hình (chưa qua Softmax)
            outputs = model(texts) 
            loss = criterion(outputs, labels) # CrossEntropyLoss sẽ tự tính Softmax bên trong
            loss.backward() 
            optimizer.step() 
            total_loss += loss.item() 
            
            # [STEP 5: PREDICTION] - [ARGMAX] Chuyển Logits thành nhãn 0/1 bằng cách lấy vị trí có giá trị lớn nhất
        ...
        # [STEP 6: EVALUATION] - Tính toán các chỉ số đánh giá chất lượng
